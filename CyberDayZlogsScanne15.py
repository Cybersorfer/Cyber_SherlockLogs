...
# 3. Nitrado Credentials (Hardcoded as requested)
...
def filter_logs(content_list, mode, target_player=None, area_coords=None, area_radius=500):
    ...
    # Expanded Vehicle Keywords
    vehicle_keys = ["vehicle", "carscript", "v3s", "ada", "olga", "gunter", "truck", "sedan", "hatchback"]
    ...
    for line in all_lines:
        ...
        if mode == "Vehicle Activity":
            ...
            should_process = True
        elif mode == "Building & Raid" and any(k in low for k in ["placed", "built", "dismantled", "folded"]):
            should_process = True
...
if "files" in st.session_state:
    selected = st.sidebar.selectbox("Select Log", [f['name'] for f in st.session_state.files])
    if st.sidebar.button("📥 Fetch & Analyze"):
        path = next(f['path'] for f in st.session_state.files if f['name'] == selected)
        data = download_nitrado_file(path)
        if data: st.session_state.current_data = [data]

st.title("🛡️ CyberDayZ Scanner v26.9")
col1, col2 = st.columns([1, 2.3])

with col1:
    # Explicitly check for mode list
    mode = st.selectbox("Search Mode", ["Vehicle Activity", "Area Activity Search", "Full Activity per Player", "Building & Raid"])
    ...
    if "results" in st.session_state:
        st.download_button("💾 Download ADM", st.session_state.dl, "FILTERED.adm")
