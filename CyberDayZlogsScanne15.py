...
if check_password():
    ...
    with st.sidebar:
        ...
        if st.session_state.all_logs:
            ...
            if selected_disp and st.button("📦 Prepare ZIP", use_container_width=True):
                ...
                # Dynamic Naming
                s_name = date_range[0].strftime("%m-%d")
                e_name = date_range[1].strftime("%m-%d")
    ...
    with col1:
        ...
        if st.session_state.track_data:
            # Dynamic Naming for Filtered ADM
            try:
                f_s = date_range[0].strftime("%m-%d")
                f_e = date_range[1].strftime("%m-%d")
